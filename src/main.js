...
let cart = [];
let cartCount = 0;
let cartTotal = 0; function toggleCart() {
...
function updateCartUI() {
  ...
  document.getElementById('cartTotal').textContent = cartTotal.toFixed(2).replace('.', ',');
  ...
  if (cartCount === 0) {
    ...
    cartItems.innerHTML = cart.map(item => `
                    <div class="flex items-center gap-4 p-4 border-b border-primary-pink">
                        ...
                        <div class="flex-1">
                            <h4 class="font-semibold text-text-dark">${item.title}</h4>
                            <p class="text-dark-pink">R$ ${item.price.toFixed(2).replace('.', ',')}</p>
                        </div>
...
document.querySelectorAll('.filter-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    document.querySelectorAll('.filter-btn').forEach(b => {
      ...
      b.classList.add('bg-light-pink', 'text-primary-brown', 'border-primary-pink');
    });


    btn.classList.remove('bg-light-pink', 'text-primary-brown', 'border-primary-pink');
